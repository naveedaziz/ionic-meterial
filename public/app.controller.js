...
var app = angular.module('appCtrl', ['ngOrderObjectBy'])
   ...
   .controller('appCtrl', function ($mdSidenav, $stateParams, $rootScope, $mdDialog, $state, $location, $http, $controller, $scope, $interval) {
      ...
      $scope.helpGet = function () {
            $http.get('https://api-global.azurewebsites.net/api/fccu_help')
               .then(function (respose) {
                 $scope.posts = respose.data;
                 for(var po in $scope.posts){
                    if ($scope.posts[po].comments && $scope.posts[po].comments != null) {
                       $scope.posts[po].comments = JSON.parse($scope.posts[po].comments);
                    }
                    if ($scope.posts[po].likes && $scope.posts[po].comments != null) {
                       $scope.posts[po].likes = JSON.parse($scope.posts[po].likes);
                    }
                 }
      ...
      $scope.helpStateLike = function (post,increment) {
         post.likes = post.likes + increment;
         $http.post('https://api-global.azurewebsites.net/api/fccu_help', {likes: post.likes,id:post.id})
            .then(function (respose) {
            }, function () {
            })

      }
      $scope.helpStateComments = function (comment) {
         $http.post('https://api-global.azurewebsites.net/api/fccu_help', { comments: JSON.stringify(comment)})
            .then(function (respose) {
            }, function () {
            })
      ...
      $scope.getBlogDetail = function () {
         ...
         // });
         $http.get('https://api-global.azurewebsites.net/api/fccu_help?id=' + $stateParams.page)
            .then(function (respose) {
               $scope.posts_detail = respose.data[0];
               if ($scope.posts_detail.comments && $scope.posts_detail.comments != '') {
                  $scope.posts_detail.comments = JSON.parse($scope.posts_detail.comments);
               }
               if ($scope.posts_detail.comments == null){
                  $scope.posts_detail.comments = [];
               }
               if ($scope.posts_detail.likes && $scope.posts_detail.likes != '') {
                  $scope.posts_detail.likes = JSON.parse($scope.posts_detail.likes);
               }
               if ($scope.posts_detail.likes == null) {
                  console.log('122')
                  $scope.posts_detail.likes = [];
               }
            }, function () {
      ...
      $scope.liked = function(){
         if ($scope.posts_detail && $scope.posts_detail.likes)
            return $scope.posts_detail.likes.filter(function (elem) { return elem.user_id == localStorage.getItem('userID') }).length;
      }
      $scope.likeEvent = function(){
         if ($scope.posts_detail.likes.length && $scope.posts_detail.likes.filter(function (elem) { return elem.user_id == localStorage.getItem('userID')}).length){
            var newLikes = [];
            for (var like in $scope.posts_detail.likes){
               if ($scope.posts_detail.likes[like].user_id != localStorage.getItem('userID'))
               newLikes.push($scope.posts_detail.likes[like])
            }
            $scope.posts_detail.likes = newLikes;
         }else{
            $scope.posts_detail.likes.push({ user_id: localStorage.getItem('userID')});
         }
         $http.post('https://api-global.azurewebsites.net/api/fccu_help', { likeEvent:true, id: $stateParams.page, likes: JSON.stringify($scope.posts_detail.likes)})
            .then(function (respose) {
      ...
      $scope.pushComments = function () {
         if (document.getElementById('commentText').value && document.getElementById('commentText').value != ''){
            ...
            $scope.posts_detail.comments.push({ createdOn: createdOn, body: document.getElementById('commentText').value,user_id:localStorage.getItem('userID')}); 
            console.log($scope.posts_detail.comments)
            document.getElementById('commentText').value = '';
            $http.post('https://api-global.azurewebsites.net/api/fccu_help', { commentEvent: true, id: $stateParams.page, comments: JSON.stringify($scope.posts_detail.comments) })
               .then(function (respose) {
               }, function () {
     ...
     $scope.currentpages = $location.$$path;;
      ...
      $scope.returnDate = function(itm,type){
         if(itm && !type){
            ...
            return moment(itm).subtract('5',"hours").format('YYYYMMDDTHHmm');
         }
  ...
  // }, 1000)
      ...
      this.grids = [
            ...
            img: 'News', name: 'News',link:'news', 
                     ...
                     {
                        img: 'Alerts', name: 'Alerts', link: 'alert',
                     }, 
                     {
      ...
      $scope.sendEmail = function(){
         $http.get('https://api-global.azurewebsites.net/api/sendgrid?to=' + $scope.user.emailAddress+'&type=register')
            .then(function (respose) {
      ...
      $scope.instagram = function(){
         $http.get('http://www.instagram.com/formanchristiancollege/media/', {
            headers: {
               'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            }
         })
